---
title: "30538 Problem Set 3: git pair"
<<<<<<< merge_conflict_name_2
author: "Mohamed Mohamed"
=======
author: "Andrew White"
>>>>>>> merge_conflict_name_1
format: 
  pdf:
    include-in-header: 
       text: |
         \usepackage{fvextra}
         \DefineVerbatimEnvironment{Highlighting}{Verbatim}{breaklines,commandchars=\\\{\}}
include-before-body:
  text: |
    \RecustomVerbatimEnvironment{verbatim}{Verbatim}{
      showspaces = false,
      showtabs = false,
      breaksymbolleft={},
      breaklines
    }
---
Partner1: 
Partner2: Mohamed Mohamed, Moham22-Star
---

```{python}
import pandas as pd
import numpy as np

np.random.seed(42)
data = pd.DataFrame(np.random.rand(100), columns=['Random Numbers'])

def preview_data(df):
    print(df.head())

preview_data(data)
```

```{python}
def describe_function(data):
    head_data = data.head()  # First object: Head of the DataFrame
    describe_data = data.describe()  # Second object: Description of the DataFrame
    
    return [head_data, describe_data]  # Return both as a list
```

